...
import { Wrench, X, Loader2, LayoutGrid, List, Search } from "lucide-react";
...
export function Toolbox() {
  ...
  const [activeFixes, setActiveFixes] = useState<Set<FixKey>>(() => new Set());
  const [fixValues, setFixValues] = useState<FixValues>(FIX_DEFAULTS);
  const [keepOriginal, setKeepOriginal] = useState(true);

  const addFix = (key: FixKey) => setActiveFixes((s) => new Set(s).add(key));
  const removeFix = (key: FixKey) =>
    setActiveFixes((s) => {
      const next = new Set(s);
      next.delete(key);
      return next;
    });
  const patchFixValues = (patch: Partial<FixValues>) => setFixValues((v) => ({ ...v, ...patch }));
  ...
  const handleStart = async () => {
    ...
    try {
      const { job_id } = await toolboxApi.start({
        ...
        audio_channel: activeFixes.has("audio") ? fixValues.audioChannel : null,
        rotate_deg: activeFixes.has("rotate") ? fixValues.rotateDeg : null,
        normalize: activeFixes.has("normalize"),
        faststart: activeFixes.has("faststart"),
        sync_offset_ms:
          activeFixes.has("sync") && fixValues.syncOffsetMs !== 0 ? fixValues.syncOffsetMs : null,
